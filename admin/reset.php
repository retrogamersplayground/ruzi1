...
if(isset($_POST['submit'])){
	...
	if(!isset($error)){
		...
		try {
			...
			$stmt = $db->prepare("UPDATE admins SET resetToken = :token, resetComplete='No' WHERE email = :email");
			$stmt->execute(array(
				':email' => $row['email'],
				':token' => $token
			));
			...
			<p>To reset your password, visit the following address: <a href='".DIR."admin/resetPassword.php?key=$token'>".DIR."admin/resetPassword.php?key=$token</a></p>";
			...
			header('Location: admin/login.php?action=reset');
			exit;
